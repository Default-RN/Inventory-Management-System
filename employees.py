...
def select_data(event,empid_entry,name_entry,email_entry,
    ...
    context=employee_treeview.item(index)
    row=context['values']
...
def update_employee(empid,name,email,gender,dob,contact,employment_type,education,work_shift,address,doj,salary,user_type,
                    password):
    ...
    if not has_special_char(password):
        ...
        try:
            ...
            address=address.strip()
            new_data=(name,email,gender,dob,contact,employment_type,education,work_shift,address,doj,salary,user_type,
                        password)
    ...
    if not selected:
        ...
        if result:
             ...
             try:
                 ...
                 connection.commit()
                 treeview_data()
                 messagebox.showinfo('Success','Selected record is deleted')
             except Exception as e:
                 messagebox.showerror('Error', f'Error due to {e}')
             finally:
                 cursor.close()
...
def employee_form(window):
    ...
    back_button=Button(top_frame,text='Back',cursor='hand2',bg='white',command=lambda: employee_frame.place_forget())
    back_button.place(x=10,y=0)
    search_frame=Frame(top_frame,bg='white')
    search_frame.pack()
    search_combobox=ttk.Combobox(search_frame,values=('EmpId','Name','Email'),font=('times new roman',12),state='readonly')
    search_combobox.set('Search By')
    ...
    show_button = Button(search_frame, text='Show All', font=('times new roman', 12),width=10,cursor='hand2',fg='white',bg='#0f4d7d',command=lambda: show_all(search_combobox.get(),search_entry.get()))
    show_button.grid(row=0, column=3)

    horizontal_scrollbar=Scrollbar(top_frame,orient=HORIZONTAL)
    vertical_scrollbar=Scrollbar(top_frame,orient=VERTICAL)
    employee_treeview=ttk.Treeview(top_frame,columns=('empid','name','email','gender','dob','contact','employment_type',
    ...
    education_options = ["B.Tech","B.Com","M.Tech","M.Com","B.Sc","M.Sc","BBA","MBA","BCA","MCA"]
    ...
    #this will open only one employee frame at a time
