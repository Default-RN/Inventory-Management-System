...
def clear(invoice_entry,name_entry,contact_entry,description_text,treeview):
    ...
    treeview.selection.remove(treeview.selection())
...
def update_supplier(invoice,name,contact,description,treeview):
    ...
    if not invoice.isdigit():
        messagebox.showerror('Validation Error', 'Invoice must contain only numeric values')
        return

    if not name.replace(" ", "").isalpha():
        messagebox.showerror('Validation Error', 'Supplier name must contain only letters')
        return

    if not contact.isdigit() or len(contact) != 10:
        messagebox.showerror('Validation Error', 'Contact must be a 10-digit number')
        return
    if not index:
        messagebox.showerror('Error','No row is selected')
    cursor,connection = connect_database()
...
def supplier_form(window):
    ...
    search_entry = Entry(search_frame, font=('times new roman', 14, 'bold'), bg='light yellow',width=10)
